...
const roleDescriptions = {
  parent: "help manage the archive and add memories for Diki",
  contributor: "add memories and help preserve Diki’s story",
...
export function buildInvitationEmail(input: InvitationEmailInput) {
  ...
  const html = `<!doctype html><html><body style="margin:0;background:#f4f0e8;color:#1f3028;font-family:Arial,sans-serif"><table role="presentation" width="100%" cellspacing="0" cellpadding="0"><tr><td style="padding:32px 16px"><table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="max-width:560px;margin:auto;background:#fffdf8;border:1px solid #ddd6c8;border-radius:24px"><tr><td style="padding:38px 32px"><p style="margin:0 0 22px;color:#426553;font-size:13px;font-weight:700;letter-spacing:1.6px;text-transform:uppercase">Everlittle · Private family archive</p><h1 style="margin:0 0 16px;font-family:Georgia,serif;font-size:35px;font-weight:500;line-height:1.1">An invitation to ${archiveName}</h1><p style="margin:0 0 14px;font-size:17px;line-height:1.6"><strong>${inviterName}</strong> has invited you to join as a <strong>${role}</strong>.</p><p style="margin:0 0 28px;color:#56635c;font-size:15px;line-height:1.65">You’ll be able to ${permission}.</p><a href="${invitationUrl}" style="display:inline-block;background:#294f3c;border-radius:12px;color:#fffdf8;font-size:15px;font-weight:700;padding:14px 22px;text-decoration:none">View invitation</a><p style="margin:28px 0 0;color:#6d746f;font-size:13px;line-height:1.6">This private link expires ${expiry} and is intended only for ${escapeHtml(input.recipient)}. If you weren’t expecting it, you can safely ignore this email.</p></td></tr></table></td></tr></table></body></html>`;
  return { html, text };
}
