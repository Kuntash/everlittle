...
export const Route = createFileRoute("/(app)/$family/kids/$child")({ component: ChildAccess });

type ChildSession = { signedIn: boolean; child?: { displayName: string; slug: string } };

function ChildAccess() {
  const { child: childSlug, family: familySlug } = Route.useParams();
  ...
  useEffect(() => {
    Promise.all([
      ...
      .then(([profiles, currentSession]) => {
        ...
        setHasOtherChildren(profiles.children.length > 1);
